    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        if (!Util.isInteger(prId)) {
            ...
            if (product == null) {
                ...
                if (user != null) { // add product to session cart -> User available in the database

                    Criteria c1 = s.createCriteria(Cart.class); //session user / product
                    c1.add(Restrictions.eq("user", user));
                    c1.add(Restrictions.eq("product", product));

                    if (c1.list().isEmpty()) { //product not available is same product id
                ...
                } else { // add product to session cart -> User not available in the database

                    HttpSession ses = request.getSession(); // session cart

                    if (ses.getAttribute("sessionCart") == null) { //session cart not available in the session
                    ...
                    } else { //session cart available in the session
                        ...
                        if (foundedCart != null) {
                            ...
                            if (Integer.parseInt(qty) <= product.getQty()) {
                                ...
                                foundedCart = new Cart(); // assign a new cart
