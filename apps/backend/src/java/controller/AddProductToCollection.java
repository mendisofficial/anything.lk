...
@WebServlet(name = "AddProductToCollection", urlPatterns = {"/AddProductToCollection"})
public class AddProductToCollection extends HttpServlet {
            ...
            try {
                ...
                if (col == null || col.getUser() == null || col.getUser().getId() != u.getId()) {
                    ...
                    if (p == null || p.getUser() == null || p.getUser().getId() != u.getId()) {
                        ...
                        if (!c.list().isEmpty()) {
                            res.addProperty("message", "Already added to collection");
                        } else {
                            CollectionProduct cp = new CollectionProduct();
                            cp.setCollection(col);
                            cp.setProduct(p);
                            s.save(cp);
                            tx.commit();
                            res.addProperty("status", true);
                            res.addProperty("message", "Added to collection");
                        }
                    }
                }
            } catch (Exception e) {
                if (tx != null) tx.rollback();
                res.addProperty("message", "Failed to add to collection");
            } finally {
                s.close();
            }
        }
